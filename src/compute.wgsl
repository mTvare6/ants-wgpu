...
fn pheromone_color(state: u32) -> vec4<f32> {
  if state == AWAY { return vec4(1.0, 0.0, 0.0, 1.0); } else { return vec4(0.0, 0.0, 1.0, 1.0); }
}
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
  ...
  let color = pheromone_color(ant.state);
  textureStore(pheromone_map, tex_coords, color);
